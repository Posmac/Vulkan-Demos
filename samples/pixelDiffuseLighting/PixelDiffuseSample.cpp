...
namespace vk
{
    bool PixelDiffuseSample::Initialize(WindowParameters& windowParams,
        ...
        std::vector<const char*> deviceExtensions)
    {
        ...
        MeshLoader::LoadMesh("models/knot.obj", true, false, false, true, model);
        ...
        if (!GetBinaryFileContents("shaders/pixelDiffuseLighting/shaderSPIRV.vert.txt", vertexShader))
        {
            return false;
        }
        ...
        if (!GetBinaryFileContents("shaders/pixelDiffuseLighting/shaderSPIRV.frag.txt", fragmentShader))
        {
            return false;
        }
    ...
    bool PixelDiffuseSample::Draw()
    {
        auto recordCommandBuffer = [&](VkCommandBuffer commandBuffer, uint32_t imageIndex, VkFramebuffer framebuffer)
        {
            ...
            ProvidePushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(float) * 4,
                &lightPosition[0]);
    ...
    bool PixelDiffuseSample::Resize()
    {
        return true;
    }

    void PixelDiffuseSample::Destroy()
    {
