...
#include "Structs/Descriptors.h"
#include "Structs/Image.h"
#include "Structs/Pipeline.h"
#include "Structs/RenderPass.h"
#include "Structs/Semaphore.h"
...
namespace vk
{
    ...
    public:
            ...
            VkQueue queue, std::vector<VkSemaphore>& semaphores)
        {
            ...
            CopyDataBetweenBuffers(cmdBuf, stagingBuffer, dstBuffer, { region });
            ...
            std::vector<CopyDescriptorInfo>& copyDescriptorsInfo)
        {
            ...
            for (auto& bufferDescriptor : bufferDescriptorsInfo)
            {
                ...
                info.pNext = nullptr;
                info.dstSet = bufferDescriptor.descriptorSet;
                info.dstBinding = bufferDescriptor.binding;
                info.dstArrayElement = bufferDescriptor.arrayElement;
                info.descriptorType = bufferDescriptor.descriptorType;
                info.descriptorCount = static_cast<uint32_t>(bufferDescriptor.bufferInfo.size());
                info.pImageInfo = nullptr;
                info.pTexelBufferView = nullptr;
                info.pBufferInfo = bufferDescriptor.bufferInfo.data();

                writeDescriptors.push_back(info);
            }
        ...
        VkFramebuffer CreateFramebuffer(VkDevice& device, VkRenderPass& renderPass, const std::vector<VkImageView>& attachments,
            const VkExtent2D framebufferSize, uint32_t layersCount)
        {
        ...
        void PrepareRenderPassForGeometryRenderAndPostProcessSubpass(VkDevice device)
        {
            ...
            VkRenderPass pass;
            CreateRenderPass(device, attachmentsDescriptions, deps, subpassDeps, pass);
        }
        ...
        void SpecifyBindingAttributeDescriptions(const std::vector<VkVertexInputBindingDescription>& bindDescriptions,
            const std::vector<VkVertexInputAttributeDescription>& attribDescriptions, VkPipelineVertexInputStateCreateInfo& info)
        {
        ...
        void SpecifyViewportAndScissorsState(ViewportInfo viewportInfo, VkPipelineViewportStateCreateInfo& info)
        {
            info =
            {
        ...
        void SpecifyRasterizationState(VkBool32 depthClampEnable,
        ...
        void SpecifyMultisamplingState(
        ...
        void SpecifyDynamicStates(std::vector<VkDynamicState> dynStates,
            ...
            std::vector<VkPushConstantRange> pushConstants)
        {
            ...
            CreatePipelineLayout(device, { layout }, pushConstants, pipelineLayout);
        ...
        VkPipeline CreateGraphicsPipelineExample(VkDevice device,
            VkPipelineCreateFlags additionalOptions,
